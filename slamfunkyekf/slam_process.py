...
from pose_ekf import PoseEKF                        # ← EKF
...
def run_slam_process(pss: ProcessSharedState) -> None:
    ...
    try:
        for raw_angles, raw_distances in lidar_driver.scan_rounds(lidar, scan_mode):
            ...
            x_mm, y_mm, theta_deg = slam.getpos()              # ← EKF: raw SLAM pose
            ekf.predict()                                       # ← EKF: grow uncertainty
            ekf.update(x_mm, y_mm, theta_deg)                  # ← EKF: fuse measurement
            sx, sy, sth = ekf.get_pose()                       # ← EKF: smoothed pose
            pss.x_mm.value = sx
            pss.y_mm.value = sy
            pss.theta_deg.value = sth
