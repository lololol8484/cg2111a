...
// IMPT notes for circular buffers:
// ((tail - head - 1) & BUFFER_MASK) is the no. of free bytes
// ((head - tail) & BUFFER_MASK) is the no. of used bytes
// & BUFFER_MASK is equivalent to % BUFFER_SIZE when BUFFER_SIZE is a power of 2
...
bool txEnqueue(const uint8_t *data, uint8_t len) {
    // TODO
    if (len > ((tx_tail - tx_head - 1) & TX_BUFFER_MASK)){
        return false;
    }
...
bool rxDequeue(uint8_t *data, uint8_t len) {
    // TODO
    if (len > ((rx_head - rx_tail) & RX_BUFFER_MASK)){
        return false;
    }
    for (uint8_t i = 0; i < len; i++){
...
ISR (USART0_RX_vect){
    uint8_t data = UDR0;
    if (((rx_tail - rx_head - 1) & RX_BUFFER_MASK) > 0){
        rx_buf[rx_head] = data;
